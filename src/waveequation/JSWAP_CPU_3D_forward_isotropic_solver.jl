...
function JSWAP_CPU_3D_forward_isotropic_solver(;nt,
    ...
    for l=1:nt-1
        ...
        @timeit ti "compute_sigma" @parallel JSWAP_CPU_3D_isotropic_forward_solver_compute_au_for_sigma(dt,dx,dy,dz,inv_Qa,lambda,mu,
        beta,
        ...
        @timeit ti "compute_sigma" @parallel JSWAP_CPU_3D_isotropic_forward_solver_compute_sigma(dt,dx,dy,dz,inv_Qa,lambda,mu,
        beta,
        sigmas11_i_j_k,
        sigmas22_i_j_k,
        sigmas33_i_j_k,
        ...
        if M11!=nothing
            if ns==1 && l<=size(Ms33_t,1)
                ...
                sigmas13_iph_j_kph[CartesianIndex.(s1,s2,s3)]=sigmas13_iph_j_kph[CartesianIndex.(s1,s2,s3)]-@ones(1,1)*dt/dx/dy/dz*Ms13_t[l];
            ...
            if ns>=2 && l<=size(Ms33_t,1)
                ...
                sigmas13_iph_j_kph[CartesianIndex.(s1,s2,s3)]=sigmas13_iph_j_kph[CartesianIndex.(s1,s2,s3)]-dt/dx/dy/dz*Ms13_t[l,:]';
        ...
        @timeit ti "compute_v" @parallel JSWAP_CPU_3D_isotropic_forward_solver_compute_v(dt,dx,dy,dz,rho,beta,

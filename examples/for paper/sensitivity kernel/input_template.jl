...
nt=20000;
# nx
nx=convert(Int64,data["nx"]);;
# ny
ny=convert(Int64,data["ny"]);;
# nz
nz=convert(Int64,data["nz"]);;
...
r1=zeros(Int64,1,length(data["r3"]));
r1[:]=data["r1"];
# receiver grid location y
r2=zeros(Int64,size(r1));
r2[:]=data["r2"];
# receiver grid location z
r3=zeros(Int64,size(r1));
r3[:]=data["r3"];
# source grid location x
s1=zeros(Int64,1,length(data["s3"]));
s1[:] .=data["s1"];
# source grid location y
s2=zeros(Int64,1,length(data["s3"]));
s2[:] .=data["s2"];
# source grid location z
s3=zeros(Int64,1,length(data["s3"]));
s3[:] .=data["s3"];;
...
M13=zeros(nt,1);
M12=zeros(nt,1);
M11[:]=rickerWave(3,dt,nt,2);
M22[:]=-rickerWave(3,dt,nt,2);
M33[:]=rickerWave(3,dt,nt,2)*0.0;
M23[:]=rickerWave(3,dt,nt,2)*0.0;
M13[:]=rickerWave(3,dt,nt,2)*0.0;
M12[:]=rickerWave(3,dt,nt,2)*0.0;
